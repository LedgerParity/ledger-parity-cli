...
## Released developer-preview delivery 2026-09-13

CLI v0.3.1-preview is published with five platform archives, checksums, examples and dashboard. Both main CI and release CI passed. The published Windows download was independently checksum-checked and executed through demo/proof creation/check. Core and connectors passed final local test/vet/build, and all seven RPC corpus tests passed. The separate contract v0.2.0-preview is published with Wasm/checksums; 13 Rust tests and five monitor/evidence tests passed in Linux CI. Protocol 28 testnet deployment, signed synthetic registration, identical post-renewal read-back, early renewal and provider fee evidence are in the contract TESTNET.md. A read-only lifetime monitor and external registry are included.

See DELIVERY.md for release/CI links, completed scope and external gates. Real operator validation, operator-owned monitoring and actual archived restoration remain outstanding; the fresh entries are live through ledger 5181347 and cannot be force-expired through the contract. The dedicated local testnet identity is in the workspace's restricted .local-checks/testnet-0.2/config directory and is not committed or distributed. No production wallet was used. The CLI remains read-only; contract signing occurred only in the separate synthetic testnet validation. Documentation and organization profile were updated and pushed.

## README image repair 2026-09-14

Fixed the organization profile's nested image path with the canonical raw GitHub URL. Regenerated banners from SVG with unclipped text and widened the lockup canvas. Root organization README now uses an opaque banner so its text remains legible in both themes. Updated images were pushed to the organization, CLI, core, connectors and documentation site. Relative README image paths resolve, seven external badge URLs return valid SVGs, and the organization profile renders the corrected URL.

The CLI packaging script now includes assets/. Version v0.3.2-preview publishes the repaired README assets without changing reconciliation behavior. CI and release run 34788259903 passed: https://github.com/LedgerParity/ledger-parity-cli/actions/runs/34788259903. Historical v0.3.1 release evidence remains above; use the newer image-fixed download.
